...
def getSummarizedData(startYear, endYear):
    ...
    with open("My Agencies.csv", "r") as csvFile:  # TODO: Make sure this file contains YOUR departments that you want data for (include headers)
    ...
    for crime in getListOfOffenses():
        ...
        while serverTimeout:
            try:
                ...
                print("*** SERVER TIMEOUT OCCURRED. Retrying in 10 Seconds! ***\n")
                time.sleep(10)
                serverTimeout = True
...
#  1. "getAllDepartments()" - To get list of ALL Law Enforcement Departments in the U.S. Writes to file. Takes a while
#                             to finish as there are thousands of agencies across the country.
...
#                             Writes to files.
#  3. "getAgencySummary()"  - To get statistics on all offenses documented for a SINGLE department. Does not write to
#                             files. Useful for debugging or quick lookup.
API_KEY = ""  # TODO: Insert YOUR API Key
BASE_URL = "https://api.usa.gov/crime/fbi/cde/"

# getAllDepartments()
getSummarizedData(startYear=2015, endYear=2023)
# getAgencySummary(ori="NM0010100", startYear=2015, endYear=2023)
